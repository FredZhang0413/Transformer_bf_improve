...
def sum_rate(H, W, sigma2=1.0):
    prod = torch.bmm(H, W)  # (B, num_users, num_users)
    signal_power = torch.abs(torch.diagonal(prod, dim1=-2, dim2=-1))**2
    interference = torch.sum(torch.abs(prod)**2, dim=-1) - signal_power
    N = sigma2
    SINR = signal_power / (interference + N) # (B, num_users)
    reward = torch.log2(1 + SINR).sum(dim=-1).mean()
    return reward
...
class BeamformingTransformer(nn.Module):
    def __init__(self, config):
        ...
        self.pos_emb = nn.Parameter(torch.zeros(config.max_seq_length, config.d_model))
    ...
    def forward(self, H, beam_history):
        ...
        norm_W_next = torch.norm(W_next, dim=1, keepdim=True)
...
class ChannelDataset(Dataset):
    def __init__(self, num_samples, num_users, num_tx):
    ...
    def __getitem__(self, idx):
        ...
        H_combined = np.stack([H_real, H_imag], axis=0)  ### Shape: (2, num_users, num_tx)       
        return torch.tensor(H_combined) ### transform to tensor
...
def train_beamforming_transformer(config):
    ...
    """
    dataset = ChannelDataset(num_samples=1000*config.batch_size, num_users=config.num_users, num_tx=config.num_tx)
    ...
    for epoch in range(config.max_epoch):
        ...
        for batch in pbar:
            ...
            # W0 = torch.randn(batch_size, config.beam_dim, device=device) # 1. Random initialization
            ...
            vec_w0 = torch.cat((torch.real(W0).reshape(-1,config.num_tx*config.num_users), torch.imag(W0).reshape(-1,config.num_tx*config.num_users)), dim=-1).reshape(-1, 2*config.num_tx*config.num_users) # (train_batches, 2*Ntx*Nrx)
            
            norm_W0 = torch.norm(vec_w0, dim=1, keepdim=True)
            ...
            for t in range(1, config.T + 1):
                beam_history = torch.stack(W_history, dim=1) ### (B, t, beam_dim)
...
if __name__ == "__main__":
    ...
    num_users = 16
    num_tx = 16
    d_state = 128 # State embedding dimension
